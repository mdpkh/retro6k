...
//efine NES_CPU      //when this is defined, the binary-coded decimal (BCD)
                     //status flag is not honored by ADC and SBC. the 2A03
                     //CPU in the Nintendo Entertainment System does not
                     //support BCD operation. DISABLED FOR RETRO6K (MDPKH)
...
void exec6502(uint32_t tickcount) {
    ...
    while ((clockticks6502 - clockgoal6502) & 0x80000000) {
        // MDPKH: while condition changed to account for overflow / wraparound
        opcode = read6502(pc++);
        status |= FLAG_CONSTANT;
